...
class Constants {
    static final String M_ENTITY = "ACCOUNT"
    static final String M_SOAP_SAVE_ACTION = "SAVE_ACCOUNT"
    static final String M_SOAP_GET_ACTION = "GET_ACCOUNT"
    static final String SapEndpoint = "BusinessPartners"
...
int pushToSAP(Message message, data) {
    try {
        def jsonRecords = []
        def item = data
        def cType  = (!item.getElementsByTagName("fcustomer_flag").item(0).textContent && item.getElementsByTagName("fsupplier_flag").item(0).textContent) ? "cSupplier" : "cCustomer"
        def activeFlag = (item.getElementsByTagName("factive_flag").item(0).textContent == "1") ? "tNO" : "tYES"
        def jsonRecord = [
            CardCode: item.getElementsByTagName("faccountid").item(0).textContent,
            CardName: item.getElementsByTagName("fname").item(0).textContent,
            CardType: cType,
            Frozen: activeFlag,
            Notes: item.getElementsByTagName("fmemo").item(0).textContent
        ]
        ...
        for (def record : jsonRecords) {
            def apiResponse 
            try {
                def getData = callServiceLayer("${Constants.SapEndpoint}('${record.CardCode}')", "GET", null, message)
                if (getData && getData instanceof Map && !getData.containsKey("error")) {
                    if (!isRecordEqual(getData, record)) {
                        apiResponse = callServiceLayer("${Constants.SapEndpoint}('${record.CardCode}')", "PATCH", record, message)
                    } 
                } else {
...
def getSAPRecord(Message message,lastBatchId,lastKey,newBatchId){
    def response = [:]
    def queryParams = "${Constants.SapEndpoint}?\$select=CardCode,CardName,CardType,Frozen,PriceListNum,Notes,U_fupdated_date" +
...
def mapSAP2WebPos(itemsToProcess) {
    ...
    itemsToProcess.each { item ->
        def sanitizedItemName = sanitizeString(item.CardName) // Sanitize ItemName
        def activeFlag = (item.Frozen == "tNO") ? "1" : "0"
        def customerFlag = (item.CardType == "cCustomer") ? "1" : "0"
        def supplierFlag = (item.CardType == "cCustomer") ? "0" : "1"
        def memo = item.Notes ?: "Ported From SAP"
        data += """
            <record>
                <fthirdpartyid>${item.CardCode}</fthirdpartyid>
                <fname>${sanitizedItemName}</fname>
                <factive_flag>${activeFlag}</factive_flag>
                <fcustomer_flag>${customerFlag}</fcustomer_flag>
                <new_fsupplier_flag>${supplierFlag}</new_fsupplier_flag>
                <new_fmemo>${memo}</new_fmemo>
                <fplevelid>${item.PriceListNum}</fplevelid>
            </record>""" 
    }
    return data
}
...
boolean isRecordEqual(def data1, def data2) {
    return data1.CardCode == data2.CardCode &&
           data1.CardName == data2.CardName &&
           data1.CardType == data2.CardType &&
           data1.Frozen == data2.Frozen &&
           data1.Notes == data2.Notes // Add other fields if needed
}
