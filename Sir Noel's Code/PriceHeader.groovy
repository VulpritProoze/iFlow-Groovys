...
class Constants {
    static final String M_ENTITY = "PRICE_HEADER"
    static final String M_SOAP_SAVE_ACTION = "SAVE_PRICE_HEADER"
    static final String M_SOAP_GET_ACTION = "GET_PRICE_HEADER"
    static final String SapEndpoint = "PriceLists"
...
def mapSAP2WebPos(itemsToProcess) {
    ...
    itemsToProcess.each { item ->
        def activeFlag = (item.Active == "tYES") ? "1" : "0"

        if(item.Active == "" && item.PriceListName == ""){
            delete = """
                <fplevel_delete_logs>
                    <fthirdpartyid>${item.PriceListNo}</fthirdpartyid>
			    </fplevel_delete_logs>
            """
        }else{
            data += """
            <record>
                <fthirdpartyid>${item.PriceListNo}</fthirdpartyid>
                <fpriceid>${item.PriceListNo}</fpriceid>
                <factive_flag>${activeFlag}</factive_flag>
                <fname>${item.PriceListName}</fname>
            </record>
            <fprice_level_list>
                <fplevelid>${item.PriceListNo}</fplevelid>
                <factive_flag>${activeFlag}</factive_flag>
                <fname>${item.PriceListName}</fname>
            </fprice_level_list>
            """ 
