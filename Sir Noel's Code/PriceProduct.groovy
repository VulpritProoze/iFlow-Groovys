...
class Constants {
    static final String M_ENTITY = "PRICE_PRODUCT"
    static final String M_SOAP_SAVE_ACTION = "SAVE_PRICE_PRODUCT"
    static final String M_SOAP_GET_ACTION = "GET_PRICE_PRODUCT"
...
int doRequest(String action, String params, Message message) {
    try {
        ...
        if (returnCode != "0") {
            if (!isPostLog) {
                def errorNodes = doc.getElementsByTagName("error")
                for (int i = 0; i < errorNodes.getLength(); i++) {
                    def errorText = errorNodes.item(i)?.textContent?.trim()
                    if (errorText) {
                        errorList << errorText
                    }
                }
                def errorString = errorList.join("; ")
                message.setProperty("errorList", errorString)

                postLog(message, "ERROR", params, errorString, "0", action)
            }
            return -1
        }
...
def getSAPRecord(Message message,lastBatchId,lastKey,newBatchId){
    def response = [:]
    def queryParams = "${Constants.SapEndpoint}?\$select=ItemCode,ItemName,InventoryUOM,ItemPrices,U_fupdated_date" +
...
def mapSAP2WebPos(itemsToProcess,Message message) {
    ...
    itemsToProcess.each { item ->
        item.ItemPrices.each { price ->
            def query = "PriceLists?\$filter=PriceListNo%20eq%20${price.PriceList}"
            def response = callServiceLayer(query, 'GET',null,message) 
            def priceListName = response.value[0].PriceListName
            def inventoryUom = item.InventoryUOM ?:"PCS"
             message.setProperty("inventoryUom", inventoryUom)
             message.setProperty("priceListName", priceListName)

            data += """
                <product>
                    <fproductid>${item.ItemCode}</fproductid>
                    <fpriceid>${price.PriceList}</fpriceid>
                    <flist_price>${price.Price}</flist_price>
                    <fuom>${inventoryUom}</fuom>
                    <price_thirdpartyid>${inventoryUom}</price_thirdpartyid>
                    <fname>${priceListName}</fname>
                </product>
            """ 
            // <product>
            //         <fproductid>${item.ItemCode}</fproductid>
