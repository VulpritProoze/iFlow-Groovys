...
class Constants {
    static final String M_ENTITY = "STOCK_STATUS"
    static final String M_SOAP_SAVE_ACTION = "SAVE_STOCK_STATUS"
    static final String SapEndpoint = "InventoryCountings"
...
def pullDataFromWebpos(Message message) {
    try {
        ...
        while (true) {
            if (requestWebPOS(lastBatchId, lastKey, newBatchId,message) < 0) return -1
            try {
                ...
                if (processDataFromWebPOS(message,message.getProperty("doRequestReturn")) < 0) return -1
...
def pullDataFromSAP(Message message) {
    try {
        ...
        while (true) {
            try {
                def list = getSAPRecord(message,lastBatchId,lastKey,newBatchId,localKey)
                message.setProperty("listsize", list?.size())
                if (list?.size() > 0) {
                    message.setProperty("list", list)
                    def itemsToProcess = list.take(100) // Example limit
                    ...
                    if (itemsToProcess?.size() > 0) {
                        // message.setProperty("lastKey", lastKey)
                        message.setProperty("itemsToProcessSize", itemsToProcess?.size())
                        // lastKey = ((lastKey as Integer) + (itemsToProcess?.size() ?: 0)).toString()
                        // message.setProperty("lastKey2", lastKey)
                        def lastSortKey = itemsToProcess[-1]?.fsort_key
                        message.setProperty("lastSortKey", lastSortKey)
                        lastKey = itemsToProcess[-1]?.fsort_key
                        if (doRequest(action, data,message) < 0) return message
                    }else{
                        message.setProperty("gothere1", "1")
                     ...
                     message.setProperty("gothere2", "1")
                    def localKeyData = buildLocalKeyData(newBatchId,"","","sap_"+Constants.M_ENTITY.toLowerCase())
                    def localKeySoapRequest = buildSoapRequest("SAVE_LOCAL_KEY", message.getProperty("W3P_ID"), message.getProperty("W3P_KEY"), localKeyData)
                    response = sendSoapRequest(localKeySoapRequest,message)
                    message.setProperty("response", response)
                ...
                message.setProperty("gothere3", "1")
...
int pushToSAP(Message message, data) {
    try {
        ...
        def jsonRecord = [
            WarehouseCode: item.getElementsByTagName("fthirdpartyid").item(0).textContent,
            WarehouseName: item.getElementsByTagName("fname").item(0).textContent,
            Inactive: activeFlag
        ]
        ...
        for (def record : jsonRecords) {
            def apiResponse 
            try {
                def getData = callServiceLayer("${Constants.SapEndpoint}('${record.WarehouseCode}')", "GET", null, message)
                if (getData && getData instanceof Map && !getData.containsKey("error")) {
                    if (!isRecordEqual(getData, record)) {
                        apiResponse = callServiceLayer("${Constants.SapEndpoint}('${record.WarehouseCode}')", "PATCH", record, message)
                    } 
                } else {
...
def getSAPRecord(Message message,lastBatchId,lastKey,newBatchId,localKey){
    def baseUrl = ""
    def response = [:]
    def inv_count_time_from = localKey['inv_count_time_from'] ?: ""
    def inv_count_time_to = localKey['inv_count_time_to'] ?: ""
    def inv_count_time_delay_min = localKey['inv_count_time_delay_min']
    message.setProperty("test", "1")
    if (lastKey == "") {
       if (!isBetweenTime(getHHMM(), inv_count_time_from,inv_count_time_to)) {
            message.setProperty("test1", "1")
            return response
        }
        if (dateDiff(lastBatchId, newBatchId) < inv_count_time_delay_min) {
            message.setProperty("test2", "1")
            return response
        }
        if (updateStockStatusTable(newBatchId) < 0) {
            message.setProperty("test3", "1")
            return response
        }
    }
    message.setProperty("test7", "1")
    def exclude_item_group_code = localKey['exclude_item_group_code']? "" : "0"
    // return message.getProperty("ServiceLayerExtension_URL")
    // baseUrl = getSAPQuery(message,localKeys['sap_stock_status_last_batchid'],localKeys['sap_stock_status_last_key']
    //     ,localKeys['sap_stock_status_new_batchid'],localKeys['inv_count_time_from'] ,localKeys['inv_count_time_to'] 
    //     ,localKeys['inv_count_time_delay_min'] )
    def queryParams = "spname=SP_SL_Extension&querytag=StockSTatus&value1=" + lastKey + "&value2=" + lastBatchId + "&value3=" + newBatchId + "&value4=" + exclude_item_group_code
    // message.setProperty("baseUrl", baseUrl)
    // message.setProperty("queryParams", queryParams)
    // def queryParams = "${Constants.SapEndpoint}?\$select=PriceListNo,PriceListName,Factor,RoundingMethod,Active,U_fupdated_date" +
    ...
    message.setProperty("response1", response)
    return response
}
double dateDiff(String yyyymmddhhmmss1, String yyyymmddhhmmss2 = getTimeStamp(), String datePart = "minutes") {
     message.setProperty("test6", "1")
    def format = "yyyyMMddHHmmss"
    
    Date t1 = Date.parse(format, yyyymmddhhmmss1 ?: "20000101120000")
    Date t2 = Date.parse(format, yyyymmddhhmmss2 ?: getTimeStamp())

    long diffMillis = t2.time - t1.time
...
String getHHMM() {
     message.setProperty("test4", "1")
    return new Date().format("HHmm")
}
boolean isBetweenTime(String hhmmCompare, String hhmmFrom, String hhmmTo) {
     message.setProperty("test5", "1")
    if (!hhmmFrom) hhmmFrom = "0000"
    if (!hhmmTo) hhmmTo = "2400"

    if (hhmmFrom < hhmmTo) {
        return hhmmCompare >= hhmmFrom && hhmmCompare <= hhmmTo
    }

    return hhmmCompare >= hhmmFrom || hhmmCompare <= hhmmTo
}
def updateStockStatusTable(sapNewBatchId){
    try{
        queryCheck = "spname=SP_SL_Extension&querytag=UpdateStockStatus&value1=" + sapNewBatchId + "&value2=&value3=&value4="
        response = callServiceLayer(queryCheck, 'GET', null, message)
        if (!response || !response.value) {
            throw new Exception("Invalid response from API UpdateStockStatus.")
            return -1
        }
        return response[0].returncode
    }catch(Exception e) {
        return -1
    }
...
def mapSAP2WebPos(itemsToProcess) {
    ...
    itemsToProcess.each { item ->
        def fqty = item.fqty / item.InvConv
        def fstnd_cost = item.fstnd_cost / item.InvConv
        data += """
            <record>
                <fthirdparty_productid>${item.fproductid}</fthirdparty_productid>
                <fthirdparty_siteid>${item.fsiteid}</fthirdparty_siteid>
                <flotno>${item.flotno}</flotno>
                <fqty>${fqty}</fqty>
                <fstnd_cost>${fstnd_cost}</fstnd_cost>
            </record>""" 
    }
...
boolean isRecordEqual(def data1, def data2) {
    return data1.PriceListName == data2.PriceListName// Add other fields if needed
}
def callServiceLayer(endpoint, method , payload = null,Message message) {
    def apiResponse = [:]
    try{
        disableSSLVerification()
        if (endpoint.contains("SP_SL_Extension")) {
            url = new URL("${message.getProperty("ServiceLayerExtension_URL")}?${endpoint}")
            message.setProperty("url", url)
        } else {
            url = new URL("${message.getProperty("ServiceLayerUrl")}/${endpoint}")
        }
        ...
        if (!responseText || responseText.trim().isEmpty()) {
            message.setProperty("apiResponse13", apiResponse)
            return [:] // Return empty map instead of parsing null/empty text
        }
        message.setProperty("apiResponse12", apiResponse)
        apiResponse = new JsonSlurper().parseText(responseText)
        message.setProperty("apiResponse11", apiResponse)
        def errorList = apiResponse.collect { it.error }.findAll { it != null }
        if (errorList) {
            message.setProperty("errorCode", errorList)
            // Handle error case here
        }
        // if (apiResponse?.error) {
        //     message.setProperty("errorCode", apiResponse?.error)
