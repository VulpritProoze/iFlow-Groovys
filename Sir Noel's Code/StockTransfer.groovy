...
class Constants {
    static final String M_ENTITY = "TRANSFER"
    static final String M_SOAP_SAVE_ACTION = "SAVE_STOCK_TRANSFER"
    static final String M_SOAP_GET_ACTION = "GET_STOCK_TRANSFER"
    static final String SapEndpoint = "StockTransfers"
...
int pushToSAP(Message message, data) {
    try {
        ...
        def queryCheck = "StockTransfers?\$filter=U_fkey%20eq%20'${fkey}'"
        ...
        for (int j = 0; j < productNodes.getLength(); j++) {
            ...
            if ((manSerNum == "Y" || manBtchNum == "Y") && product.getElementsByTagName("flotno").item(0)?.textContent?.trim() == "") {
                lotno = message.getProperty("default_lotno")?.trim() // Ensure it's trimmed and null-safe

                if (!lotno) { // Checks both null and empty values
        ...
        def jsonRecord = [
            ...
            Comments: memo,
            JournalMemo: jMemo,
            FromWarehouse: item.getElementsByTagName("fthirdparty_siteid").item(0).textContent,
            ToWarehouse: item.getElementsByTagName("fthirdparty_dst_siteid").item(0).textContent,
            AuthorizationCode: item.getElementsByTagName("fkey").item(0).textContent,
            StockTransferLines: productList
        ]
