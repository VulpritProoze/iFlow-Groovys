...
def Message processData(Message message) {
    ...
    try {
        ...
        if (creds?.status != 1) {
            ...
            logger.logBoth(new LogRequest(stepName: "${Constants.STEP_NAME}_CREDS_FAILURE", title: Constants.LOG_RECID, status: "ERROR", inputPayload: payload, outputPayload: "Missing Service Layer credentials: ${creds?.message ?: 'unknown'}"))
            return message
        }
        ...
        for (int r = 0; r < records.size(); r++) {
            ...
            if (fbase) {
                ...
                if (baseRes?.status == 1) {
                    logger.logBoth(new LogRequest(stepName: "${Constants.STEP_NAME}_UOM_RESOLVE", title: Constants.LOG_RECID, status: "OK", inputPayload: "Resolve BaseUoM: ${fbase}", outputPayload: "${baseRes.message} \n\n${baseRes.payload}"))
                } else {
